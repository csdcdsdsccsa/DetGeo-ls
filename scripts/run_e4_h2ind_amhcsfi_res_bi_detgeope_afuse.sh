#!/usr/bin/env bash
# Bi-Res + DetGeo PE with Stage-3/4 adaptive fusion and one detector head.
set -euo pipefail
cd "$(dirname "$0")/.."

PYTHON_BIN="${PYTHON_BIN:-/root/miniconda3/envs/detgeo/bin/python}"
NAME="trogeo_ms_e4_amhcsfi_res_bi_afuse_detgeope_naturalrng_swin_t_drone_seed2024"
COMMON=(
  ...
  --trogeo_ms_det_variant h2_ind_amhcsfi_res_bi_afuse
...
PYTHONPATH=. "$PYTHON_BIN" tools/test_e4_amhcsfi_res_bi_afuse.py
