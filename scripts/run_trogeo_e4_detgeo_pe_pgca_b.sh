#!/usr/bin/env bash
# Strict E4: original DetGeo input PE + PGCA-B Conv bias / dynamic lambda.
set -euo pipefail
cd "$(dirname "$0")/.."
PYTHON_BIN="${PYTHON_BIN:-/root/miniconda3/envs/detgeo/bin/python}"
NAME="trogeo_ms_e4_detgeo_pe_pgca_b_dynamic_swin_t_drone_seed2024"
COMMON=(--gpu 0 --num_workers 24 --batch_size 7 --emb_size 768 --img_size 1024
  ...
  --seed 2024 --beta 1.0 --print_freq 50 --trogeo_ms_det_variant h2_ind_pgca_b_dynamic
  --trogeo_position_mode detgeo)

PYTHONPATH=. "$PYTHON_BIN" tools/test_trogeo_e4_detgeo_pe_pgca_b.py --batch_size 7
