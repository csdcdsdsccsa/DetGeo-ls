#!/usr/bin/env bash
# Strict E4 two-scale Query-PE/LE runs: validation selects best, then one test.
set -euo pipefail
cd "$(dirname "$0")/.."
...
PYTHONPATH=. "$PYTHON_BIN" tools/test_trogeo_query_pe_2scale.py --batch_size 7

for VARIANT in h2_ind_pe_ln_amp_kv h2_ind_pe_ln_amp_key h2_ind_pe_all_add \
  h2_ind_pe_all_key h2_ind_le_stage2_res; do
  NAME="trogeo_ms_${VARIANT}_swin_t_drone_seed2024"
