#!/usr/bin/env bash
# Runs after E8: sanity first, then five ordinary equal-three-head-loss runs.
set -euo pipefail
cd "$(dirname "$0")/.."
...
for VARIANT in h2_ind_3scale_pe_ln_amp h2_ind_3scale_pe_all_add h2_ind_3scale_pe_all_key \
  h2_ind_3scale_le_ind_res h2_ind_3scale_le_stage2_res; do
