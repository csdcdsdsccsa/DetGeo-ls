...
GPU="${GPU:-0}"
SPLIT_DIR="data/CVOGL_SVI/cv5_grouped"
test -f "${SPLIT_DIR}/manifest.json"
for FOLD in 1 2 3 4 5; do
  NAME="detgeo_square_groupcv5_svi_f${FOLD}_seed13"
  ...
  PYTHONPATH=. "$PYTHON_BIN" train.py --gpu "$GPU" --num_workers 24 --max_epoch 25 --lr 1e-4 \
    --batch_size 8 --emb_size 512 --img_size 1024 --data_root data --data_name CVOGL_SVI \
