#!/usr/bin/env bash
# A3-Bi: Stage4 -> Stage3 -> Stage4 guidance with shared CA4 + CSFI + dual heads.
set -euo pipefail
cd "$(dirname "$0")/.."

PYTHON_BIN="${PYTHON_BIN:-/root/miniconda3/envs/detgeo/bin/python}"
NAME="trogeo_ms_e4_h2_ind_csfi_bi_standardrng_swin_t_drone_seed2024"
...
PYTHONPATH=. "$PYTHON_BIN" train.py "${COMMON[@]}" --max_epoch 25 --lr 1e-4 \
  --trogeo_ms_det_variant h2_ind_csfi_bi --savename "$NAME"
PYTHONPATH=. "$PYTHON_BIN" train.py "${COMMON[@]}" \
  --trogeo_ms_det_variant h2_ind_csfi_bi \
