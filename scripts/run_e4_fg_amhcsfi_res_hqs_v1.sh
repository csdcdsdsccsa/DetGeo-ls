...
NAME="trogeo_ms_e4_${VARIANT}_hqs_v1_seed2024"
[[ -f "$BASE_CKPT" ]] || { echo "Missing FG-Res checkpoint: $BASE_CKPT" >&2; exit 1; }
PYTHONPATH=. "$PYTHON_BIN" tools/test_hqs_v1.py
PYTHONPATH=. "$PYTHON_BIN" train.py \
  ...
  --hqs_v1 --hqs_lr 1e-3 --hqs_temperature 0.15 --hqs_gap_delta 0.30 \
...
PYTHONPATH=. "$PYTHON_BIN" train.py \
  ...
  --trogeo_ms_det_variant "$VARIANT" --standard_rng --seed 2024 --beta 1.0 --print_freq 50 \
  --coarse_loss_weight 0.2 --coarse_sigma 1.5 --fine_sigma 3.0 \
  --hqs_v1 --pretrain "saved_models/${NAME}_model_best.pth.tar" --test --savename "${NAME}_test"
