#!/usr/bin/env bash
# Bi-Res + DADPE-B / MS-DADPE-D under ordinary natural RNG.
set -euo pipefail
cd "$(dirname "$0")/.."

PYTHON_BIN="${PYTHON_BIN:-/root/miniconda3/envs/detgeo/bin/python}"
VARIANT="h2_ind_amhcsfi_res_bi"
COMMON=(
  --gpu 0 --num_workers 24 --batch_size 7 --emb_size 768 --img_size 1024
  --data_root data --data_name CVOGL_DroneAerial --trogeo_backbone swin_t
  --trogeo_ms_det_variant "$VARIANT"
  --trogeo_position_mode detgeo --trogeo_click_map_mode distance
  --standard_rng --seed 2024 --beta 1.0 --print_freq 50
  --coarse_loss_weight 0.2 --coarse_sigma 1.5 --fine_sigma 3.0
)

run_experiment() {
  local mode="$1"
  local name="$2"
  PYTHONPATH=. "$PYTHON_BIN" train.py "${COMMON[@]}" --dadpe_mode "$mode" \
    --max_epoch 25 --lr 1e-4 --savename "$name"
  PYTHONPATH=. "$PYTHON_BIN" train.py "${COMMON[@]}" --dadpe_mode "$mode" \
    --pretrain "saved_models/${name}_model_best.pth.tar" --test --savename "${name}_test"
  rm -f "saved_models/${name}_checkpoint.pth.tar"
}

PYTHONPATH=. "$PYTHON_BIN" tools/test_e4_dadpe.py
PYTHONPATH=. "$PYTHON_BIN" tools/test_e4_amhcsfi_res.py --batch_size 2
PYTHONPATH=. "$PYTHON_BIN" tools/test_e4_cross_scale_hier.py --batch_size 7

run_experiment input "trogeo_ms_e4_amhcsfi_res_bi_dadpe_b_naturalrng_swin_t_drone_seed2024"
run_experiment multiscale "trogeo_ms_e4_amhcsfi_res_bi_msdadpe_d_naturalrng_swin_t_drone_seed2024"
