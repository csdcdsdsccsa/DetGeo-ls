#!/usr/bin/env bash
# P1: strict A0 h2_ind plus position-guided Query refinement attention.
set -euo pipefail
cd "$(dirname "$0")/.."

PYTHON_BIN="${PYTHON_BIN:-/root/miniconda3/envs/detgeo/bin/python}"
NAME="trogeo_ms_e4_h2_ind_pqra_standardrng_swin_t_drone_seed2024"
...
PYTHONPATH=. "$PYTHON_BIN" tools/test_e4_pqra.py --batch_size 7
PYTHONPATH=. "$PYTHON_BIN" train.py "${COMMON[@]}" --max_epoch 25 --lr 1e-4 \
  --trogeo_ms_det_variant h2_ind_pqra --savename "$NAME"
PYTHONPATH=. "$PYTHON_BIN" train.py "${COMMON[@]}" --trogeo_ms_det_variant h2_ind_pqra \
