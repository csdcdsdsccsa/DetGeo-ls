#!/usr/bin/env bash
# E4 H2-Ind + CSFI + Stage4 coarse-to-Stage3 fine guidance, ordinary standard_rng.
set -euo pipefail
cd "$(dirname "$0")/.."

PYTHON_BIN="${PYTHON_BIN:-/root/miniconda3/envs/detgeo/bin/python}"
NAME="trogeo_ms_e4_h2_ind_csfi_cg_standardrng_swin_t_drone_seed2024"
...
PYTHONPATH=. "$PYTHON_BIN" train.py "${COMMON[@]}" --max_epoch 25 --lr 1e-4 \
  --trogeo_ms_det_variant h2_ind_csfi_cg --savename "$NAME"
PYTHONPATH=. "$PYTHON_BIN" train.py "${COMMON[@]}" \
  --trogeo_ms_det_variant h2_ind_csfi_cg \
