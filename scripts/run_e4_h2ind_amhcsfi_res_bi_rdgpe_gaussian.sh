#!/usr/bin/env bash
# Bi-Res + Gaussian map + RDG-PE.  Only the front-end position encoder differs.
set -euo pipefail
cd "$(dirname "$0")/.."
PYTHON_BIN="${PYTHON_BIN:-/root/miniconda3/envs/detgeo/bin/python}"
NAME="trogeo_ms_e4_amhcsfi_res_bi_rdgpe_gaussian_sigma25_naturalrng_swin_t_drone_seed2024"
COMMON=(--gpu 0 --num_workers 24 --batch_size 7 --emb_size 768 --img_size 1024
  ...
  --trogeo_ms_det_variant h2_ind_amhcsfi_res_bi --trogeo_position_mode rdg
...
PYTHONPATH=. "$PYTHON_BIN" tools/test_e4_dg_pe.py
