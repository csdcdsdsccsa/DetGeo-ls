...
NAME="fullmodel_bires_amhcsfi_res_hisym_crgpe_core25x50_outer50x100_swin_b_svi_naturalrng_seed2024"
COMMON=(
  --gpu 0,1 --num_workers 24 --batch_size 7 --emb_size 768 --img_size 1024
  --data_root data --data_name CVOGL_SVI
  --trogeo_backbone swin_b --trogeo_ms_det_variant h2_ind_amhcsfi_res_bi
...
if [[ -e "saved_models/${NAME}_model_best.pth.tar" ]]; then echo "Refusing to overwrite ${NAME}" >&2; exit 1; fi
PYTHONPATH=. "$PYTHON_BIN" tools/test_fullmodel_swin_b.py
...
echo '[Swin-B Full Model] CVOGL_SVI COMPLETE'
