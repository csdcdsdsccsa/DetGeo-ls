...
NAME="fullmodel_bires_amhcsfi_res_hisym_crgpe_core25x50_outer50x100_resnet50_svi_naturalrng_seed2024"
COMMON=(
...
if [[ -e "saved_models/${NAME}_model_best.pth.tar" ]]; then
  echo "Refusing to overwrite existing checkpoint: ${NAME}" >&2; exit 1
fi
...
echo '[ResNet50 Full Model] CVOGL_SVI COMPLETE'
