#!/usr/bin/env bash
# A3-FG: Stage3 -> Stage4 fine guidance + CSFI + independent dual heads.
set -euo pipefail
cd "$(dirname "$0")/.."

PYTHON_BIN="${PYTHON_BIN:-/root/miniconda3/envs/detgeo/bin/python}"
NAME="trogeo_ms_e4_h2_ind_csfi_fg_standardrng_swin_t_drone_seed2024"
COMMON=(--gpu 0 --num_workers 24 --batch_size 7 --emb_size 768 --img_size 1024
  ...
  --seed 2024 --beta 1.0 --print_freq 50 --coarse_loss_weight 0.2 --fine_sigma 3.0)
...
PYTHONPATH=. "$PYTHON_BIN" train.py "${COMMON[@]}" --max_epoch 25 --lr 1e-4 \
  --trogeo_ms_det_variant h2_ind_csfi_fg --savename "$NAME"
PYTHONPATH=. "$PYTHON_BIN" train.py "${COMMON[@]}" \
  --trogeo_ms_det_variant h2_ind_csfi_fg \
