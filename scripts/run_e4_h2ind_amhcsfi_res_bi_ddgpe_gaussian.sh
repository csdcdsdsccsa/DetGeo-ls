#!/usr/bin/env bash
# Bi-Res + Gaussian map + DDG-PE.  DG plus only second-order radial term Gr.
set -euo pipefail
cd "$(dirname "$0")/.."
PYTHON_BIN="${PYTHON_BIN:-/root/miniconda3/envs/detgeo/bin/python}"
NAME="trogeo_ms_e4_amhcsfi_res_bi_ddgpe_gaussian_sigma25_naturalrng_swin_t_drone_seed2024"
COMMON=(--gpu 0 --num_workers 24 --batch_size 7 --emb_size 768 --img_size 1024
  ...
  --trogeo_ms_det_variant h2_ind_amhcsfi_res_bi --trogeo_position_mode ddg
  --trogeo_click_map_mode gaussian --gaussian_sigma 25 --dadpe_mode none --amr_pe_mode none
  --standard_rng --seed 2024 --beta 1.0 --print_freq 50 --coarse_loss_weight 0.2 --coarse_sigma 1.5 --fine_sigma 3.0)
PYTHONPATH=. "$PYTHON_BIN" tools/test_e4_dg_pe.py --cuda
