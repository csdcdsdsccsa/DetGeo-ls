...
NAME="trogeo_ms_e4_h2_ind_amhcsfi_bi_standardrng_swin_t_drone_seed2024"
...
PYTHONPATH=. "$PYTHON_BIN" tools/test_e4_mhcsfi.py --batch_size 2
PYTHONPATH=. "$PYTHON_BIN" train.py "${COMMON[@]}" --max_epoch 25 --lr 1e-4 \
  --trogeo_ms_det_variant h2_ind_amhcsfi_bi --savename "$NAME"
PYTHONPATH=. "$PYTHON_BIN" train.py "${COMMON[@]}" --trogeo_ms_det_variant h2_ind_amhcsfi_bi \
