...
from model.anchorfree import build_target_anchorfree, anchorfree_loss, eval_anchorfree_acc
...
def main():
    ...
    parser.add_argument('--gpu', default='0', help='single GPU id')
    ...
    ## Model
    model = DetGeo(detector_head=args.detector_head).cuda()
    ...
    if args.test:
        ...
        for epoch in range(args.max_epoch):
            ...
            save_checkpoint({
                ...
                'detector_head': args.detector_head,
...
def train_epoch(train_loader, model, optimizer, epoch, args):
    ...
    for batch_idx, (query_imgs, rs_imgs, mat_clickxy, ori_gt_bbox, _) in enumerate(train_loader):
        ...
        predictions, _ = model(query_imgs, rs_imgs, mat_clickxy)
        if args.detector_head == 'yolo':
            pred_anchor = predictions.view(predictions.shape[0], 9, 5,
            ...
            accu_list, accu_center, iou, _, _, _ = eval_iou_acc(
                pred_anchor, ori_gt_bbox, anchors_full, best_anchor_gi_gj[:, 1],
                best_anchor_gi_gj[:, 2], args.img_size, iou_threshold_list=[0.5]
            )
        else:
            pred_heatmap, pred_bbox = predictions
            gt_heatmap, gt_bbox, mask = build_target_anchorfree(
                ori_gt_bbox, pred_heatmap.shape[2], pred_heatmap.shape[3], args.img_size
            )
            loss_cls, loss_geo = anchorfree_loss(
                pred_heatmap, pred_bbox, gt_heatmap, gt_bbox, mask
            )
            accu_list, accu_center, iou, _, _, _ = eval_anchorfree_acc(
                pred_heatmap, pred_bbox, ori_gt_bbox, args.img_size,
        ...
        if batch_idx % args.print_freq == 0:
            classification_label = 'Heatmap' if args.detector_head == 'smgeo' else 'Cls'
            geometry_label = 'BBox' if args.detector_head == 'smgeo' else 'Geo'
            print_str = 'Epoch: [{0}][{1}/{2}]\t' \
                'Time {batch_time.val:.3f} ({batch_time.avg:.3f})\t' \
                'Loss {loss.val:.4f} ({loss.avg:.4f})\t' \
                '{geometry_label} Loss {geo.val:.4f} ({geo.avg:.4f})\t' \
                '{classification_label} Loss {cls.val:.4f} ({cls.avg:.4f})\t' \
                ...
                .format( \
                    ...
                    geometry_label=geometry_label, classification_label=classification_label)
            print(print_str)
...
def test_epoch(data_loader, model, args):
    ...
    for batch_idx, (query_imgs, rs_imgs, mat_clickxy, ori_gt_bbox, _) in enumerate(data_loader):
        ...
        with torch.no_grad():
            predictions, attn_score = model(query_imgs, rs_imgs, mat_clickxy)
        if args.detector_head == 'yolo':
            pred_anchor = predictions.view(predictions.shape[0], 9, 5,
                                           predictions.shape[2], predictions.shape[3])
            ...
            pred_heatmap, pred_bbox = predictions
            accu_list, accu_center, iou, each_acc_list, _, _ = eval_anchorfree_acc(
                pred_heatmap, pred_bbox, ori_gt_bbox, args.img_size,
                iou_threshold_list=[0.5, 0.25]
            )
