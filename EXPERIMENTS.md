...
val records; official test records are never input to fold generation. Each
group is a connected component of the query--satellite bipartite graph, so no
query or satellite identity appears on both sides of a fold's train/validation
split. Folds use seed 2024 and balanced greedy allocation by component size.

The ten runs are independent ImageNet-pretrained Swin-T Full Models with 25
epochs and natural RNG. Training selects only by each fold's validation set.
