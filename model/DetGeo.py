...
class DetGeo(nn.Module):
    def __init__(self, emb_size=512, leaky=True, detector_head='yolo'):
        super(DetGeo, self).__init__()
        if detector_head not in ('yolo', 'smgeo'):
            raise ValueError("detector_head must be 'yolo' or 'smgeo'")
        ...
        if detector_head == 'yolo':
            ...
            self.fcn_out = AnchorFreeHead(emb_size)
