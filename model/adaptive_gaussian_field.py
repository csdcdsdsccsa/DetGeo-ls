...
class AdaptiveGaussianField(nn.Module):
    ...
    def forward(self, rgb, click_xy, return_aux=False):
        ...
        base = isotropic[:, self.base_index:self.base_index + 1]
        ...
        if self.mode == 'msg':
            ...
            a = self.a_max * torch.tanh(shape[:, 0])
            theta = 0.5 * math.pi * torch.tanh(shape[:, 1])
