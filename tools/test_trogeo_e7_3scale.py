...
def main():
    ...
    model = torch.nn.DataParallel(
        TROGeoMSDetectionAblation(variant='h2_ind_3scale')
    ).cuda().train()
    ...
    geo_loss, cls_loss = three_head_yolo_loss(*heads, gt_xyxy, anchors, 1024)
    ...
    print('trogeo_e7_3scale batch={} loss={:.8f} total_params={} peak_mib={:.1f}'.format(
        batch, loss.item(), sum(parameter.numel() for parameter in model.parameters()),
