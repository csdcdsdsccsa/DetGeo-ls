"""Two-step GPU sanity checks for strict E4 two-scale Query-PE/LE variants."""
...
def check_variant(variant, batch):
    ...
    # SpatialTransformer.proj_out is zero-initialized.  After its first update,
    # the position branch must receive a nonzero gradient on this second step.
    predictions, _ = model(query, reference, click)
    ...
    if variant in VARIANTS[:-1]:
        for name in ('pe_proj3', 'pe_proj4', 'pe_alpha3', 'pe_alpha4'):
        ...
        if variant.startswith('h2_ind_pe_ln_amp_'):
            for name in ('pe_norm3', 'pe_norm4'):
                if not has_gradient(getattr(module, name).weight):
                    raise RuntimeError('{} {} has zero second-step gradient'.format(variant, name))
    else:
        if hasattr(module, 'le_stage3') or hasattr(module, 'le_stage4'):
            raise RuntimeError('{} must only own le_stage2'.format(variant))
        if not has_gradient(module.le_stage2[0].weight):
            raise RuntimeError('{} le_stage2 has zero second-step gradient'.format(variant))
        for name in ('le_beta3_logit', 'le_beta4_logit'):
            if not has_gradient(getattr(module, name)):
                raise RuntimeError('{} {} has zero second-step gradient'.format(variant, name))
    print('query_pe_2scale_sanity variant={} loss1={:.6f} loss2={:.6f} params={} peak_mib={:.1f}'.format(
...
def check_e4_contract(batch):
    """Regression guard: new routing must not turn the E4 baseline into E7."""
    torch.cuda.empty_cache()
    model = torch.nn.DataParallel(TROGeoMSDetectionAblation(variant='h2_ind')).cuda().eval()
    with torch.no_grad():
        predictions, _ = model(
    ...
    module = model.module
    if set(predictions) != {'stage3', 'stage4'}:
        raise RuntimeError('E4 baseline routing changed to {}'.format(set(predictions)))
    if any(value.shape != (batch, 45, 64, 64) for value in predictions.values()):
        raise RuntimeError('E4 baseline emitted invalid prediction shape')
    if hasattr(module, 'cvopm_stage2') or hasattr(module, 'det_head_stage2'):
        raise RuntimeError('E4 baseline unexpectedly owns Stage2 detection modules')
    print('query_pe_2scale_e4_contract batch={} outputs=strict_stage3_stage4'.format(batch), flush=True)
...
def main():
    ...
    check_e4_contract(args.batch_size)
